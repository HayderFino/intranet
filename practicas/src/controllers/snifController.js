...
const FORBIDDEN_EXTS = ['.exe', '.bat', '.cmd', '.vbs', '.sh', '.ps1', '.msi', '.com'];
...
const SnifController = {
    ...
    getAll: (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Error al obtener listado SNIF', error: error.message });
        }
    },
    ...
    create: (req, res) => {
        try {
            ...
            if (!name || !name.trim()) {
                ...
                return res.status(400).json({ message: 'El nombre del archivo es obligatorio.' });
            }
            ...
            if (req.file.size > MAX_SIZE_BYTES) {
                ...
                return res.status(400).json({ message: 'El archivo supera el tamaño máximo permitido (20 MB).' });
            }
            ...
            if (!fs.existsSync(destDir)) {
                fs.unlinkSync(req.file.path);
                return res.status(500).json({ message: 'La carpeta de destino SNIF no existe en el servidor.' });
            }
            ...
            if (id) {
                ...
                res.status(500).json({ message: 'No se pudo insertar el archivo en el HTML del SNIF.' });
            }
        } catch (error) {
            console.error('[SNIF] create error:', error.message);
            res.status(500).json({ message: 'Error al crear el archivo SNIF.', error: error.message });
        }
    },
    ...
    update: (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Error al actualizar el archivo SNIF.', error: error.message });
        }
    },
    ...
    delete: (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Error al eliminar el archivo SNIF.', error: error.message });
        }
    }
};
