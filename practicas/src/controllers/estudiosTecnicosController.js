...
const FORBIDDEN_EXTS = ['.exe', '.bat', '.cmd', '.js', '.sh', '.ps1', '.vbs', '.msi'];
...
const estudiosTecnicosController = {
    ...
    create: async (req, res) => {
        try {
            ...
            const newItem = model.create({ title, type, date, description, filename: finalName });
    ...
    update: async (req, res) => {
        try {
            ...
            const success = model.update(id, { title, type, date, description, filename });
            if (success) res.json({ status: 'ok', message: 'Actualizado correctamente' });
    ...
    delete: async (req, res) => {
        try {
            ...
            if (success) res.json({ status: 'ok', message: 'Eliminado correctamente' });
            else res.status(404).json({ status: 'error', message: 'No encontrado' });
        } catch (err) {
            res.status(500).json({ status: 'error', message: err.message });
        }
