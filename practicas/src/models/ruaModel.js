...
const RuaModel = {
    getAll: () => {
        ...
        const gridRegex = /<div class="pdf-grid" id="rua-docs-grid">([\s\S]*?)<!-- END_RUA_GRID -->/;
    ...
    create: (data) => {
        ...
        content = content.replace(/(<!-- END\_RUA\_GRID -->)/, newItemHtml + '\n                $1');
    ...
    delete: (id) => {
        ...
        if (targetCardIndex !== -1) {
            ...
            if (hrefMatch && hrefMatch[1] && !hrefMatch[1].startsWith('http') && hrefMatch[1] !== '#') {
                ...
                const absolutePath = path.resolve(path.dirname(RUA_HTML_PATH), relativePath);
