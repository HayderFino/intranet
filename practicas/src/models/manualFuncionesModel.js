...
class ManualFuncionesModel {
    constructor() {
        this.htmlPath = path.join(__dirname, '../../header_menu/cas/manual-funciones.html');
        this.dataDir = path.join(__dirname, '../../data/menu header/la cas/talento humano/Manual de Funciones');
    }
    ...
    getAll() {
        ...
        const gridRegex = /id="manual-funciones-grid"[\s\S]*?>([\s\S]*?)<\/section>/i;
        ...
        if (gridMatch) {
            ...
            const cardRegex = /<a\s+[^>]*?class="pdf-folder-card"[^>]*?>([\s\S]*?)<\/a>/gi;
            let cardMatch;
            ...
            while ((cardMatch = cardRegex.exec(gridContent)) !== null) {
                ...
                const btnMatch = /<span [^>]*?class="btn-pdf-download"[^>]*?>([\s\S]*?)<\/span>/i.exec(inner);

                if (idMatch) {
                    items.push({
                        id: idMatch[1],
                        name: nameMatch ? this._decodeHTML(nameMatch[1].trim()) : 'Sin nombre',
    ...
    create(item) {
        ...
        if (['.xls', '.xlsx'].includes(ext)) { iconColor = '#1d6f42'; btnBg = '#1d6f42'; typeStr = 'Descargar XLSX'; }
        else if (['.doc', '.docx'].includes(ext)) { iconColor = '#2b579a'; btnBg = '#2b579a'; typeStr = 'Descargar DOCX'; }
        else if (['.ppt', '.pptx'].includes(ext)) { iconColor = '#d24726'; btnBg = '#d24726'; typeStr = 'Descargar PPT'; }
        ...
        if (index !== -1) {
            ...
            content = content.substring(0, insertIndex) + '\n' + cardHtml + content.substring(insertIndex);
    ...
    update(id, newData) {
        ...
        const cardRegex = new RegExp(`<a [^>]*?data-id="${escapedId}"[^>]*?>[\\s\\S]*?<\\/a>`, 'gi');
        const match = cardRegex.exec(content);

        if (match) {
            const oldCardHtml = match[0];
            const name = newData.name || (/<h4>([\s\S]*?)<\/h4>/i.exec(oldCardHtml)?.[1] || 'Sin nombre');
            const fileUrl = newData.filename ? `../../data/menu header/la cas/talento humano/Manual de Funciones/${newData.filename}` : (/href="([^"]*)"/i.exec(oldCardHtml)?.[1] || '#');
    ...
    delete(id) {
        ...
        if (match) {
            ...
            if (href && href !== '#' && !href.startsWith('http')) {
                const absPath = path.resolve(path.dirname(this.htmlPath), href.split('?')[0]);
