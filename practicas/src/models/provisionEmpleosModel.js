...
class ProvisionEmpleosModel {
    ...
    update(id, newData) {
        ...
        if (match) {
            ...
            const type = newData.type || (/class="doc-type">([\s\S]*?)<\/span>/i.exec(oldHtml)?.[1] || 'PROVISI&Oacute;N DE EMPLEO');
            ...
            const href = newData.filename ? `../../data/menu header/la cas/talento humano/Provision de empleos/${newData.filename}` : (/href="([^"]*)"/i.exec(oldHtml)?.[1] || '#');
    ...
    delete(id) {
        ...
        if (match) {
            ...
            if (hrefMatch && hrefMatch[1] && hrefMatch[1] !== '#' && !hrefMatch[1].startsWith('http')) {
                const relativePath = decodeURIComponent(hrefMatch[1].split('?')[0]).trim();
    ...
    _decodeHTML(html) {
        return html
            .replace(/&iacute;/g, 'í').replace(/&aacute;/g, 'á').replace(/&uacute;/g, 'ú').replace(/&ó;/g, 'ó').replace(/&oacute;/g, 'ó').replace(/&é;/g, 'é').replace(/&eacute;/g, 'é')
