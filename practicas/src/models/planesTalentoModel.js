...
class PlanesTalentoModel {
    constructor() {
        ...
        this.baseDataDir = path.join(__dirname, '../../data/menu header/la cas/talento humano/Planes');
    ...
    getAll() {
        ...
        Object.entries(this.categories).forEach(([categoryName, listId]) => {
            const listRegex = new RegExp(`id="${listId}"[\\s\\S]*?>([\\s\\S]*?)<\\/(?:div|ul)>`, 'i');
            const listMatch = listRegex.exec(content);

            if (listMatch) {
                const listContent = listMatch[1];
                const itemRegex = /<a\s+[^>]*?class="plan-item"[^>]*?>([\s\S]*?)<\/a>/gi;
                let itemMatch;
                ...
                while ((itemMatch = itemRegex.exec(listContent)) !== null) {
                    ...
                    if (idMatch) {
                        items.push({
                            id: idMatch[1],
                            name: this._decodeHTML(nameMatch ? nameMatch[1].trim() : 'Sin nombre'),
    ...
    update(id, newData) {
        ...
        const itemRegex = new RegExp(`<a [^>]*?data-id="${escapedId}"[^>]*?>[\\s\\S]*?<\\/a>`, 'gi');
        const match = itemRegex.exec(content);

        if (match) {
            const oldHtml = match[0];
            const name = newData.name || 'Documento';
            const href = newData.filename ? `../../data/menu header/la cas/talento humano/Planes/${newData.category}/${newData.filename}` : (/href="([^"]*)"/i.exec(oldHtml)?.[1] || '#');
