...
const RespelModel = {
    getAll: (section = 'documentos') => {
        ...
        if (section === 'documentos') {
            const gridRegex = /<div class="pdf-grid" id="respel-docs-grid">([\s\S]*?)<!-- END_RESPEL_GRID -->/;
            const gridMatch = gridRegex.exec(content);
            if (gridMatch) {
                const gridContent = gridMatch[1];
                const cardRegex = /<div class="pdf-folder-card">([\s\S]*?)<\/div>\s*(?=<div class="pdf-folder-card">|<!-- END_RESPEL_GRID -->|$)/g;
                let match;
                while ((match = cardRegex.exec(gridContent)) !== null) {
                    ...
                    if (idMatch) {
                        items.push({
                            id: idMatch[1],
                            name: (nameMatch ? nameMatch[1].trim() : 'Documento').replace(/&oacute;/g, 'ó').replace(/&nbsp;/g, ' '),
    ...
    create: (section, data) => {
        ...
        if (section === 'documentos') {
            ...
            content = content.replace(/(<!-- END\_RESPEL\_GRID -->)/, newItemHtml + '\n                $1');
        } else if (section === 'empresas') {
            const bgClass = data.isAlternate ? 'style="background:#f9fbe7;"' : 'style="background:#fff;"';
            const newItemHtml = `
                            <tr ${bgClass}>
            ...
            content = content.replace(/<tbody>([\s\S]*?)<\/tbody>/, (match, items) => {
    ...
    delete: (section, id) => {
        ...
        if (section === 'documentos') {
            ...
            if (targetCardIndex !== -1) {
                ...
                if (hrefMatch && hrefMatch[1] && !hrefMatch[1].startsWith('http') && hrefMatch[1] !== '#') {
                    const relativePath = hrefMatch[1];
                    const absolutePath = path.resolve(path.dirname(RESPEL_HTML_PATH), relativePath);
            ...
            const itemRegex = new RegExp(`<tr[^>]*>\\s*<td[^>]*data-id="${id}"[\\s\\S]*?<\\/tr>`, 'g');
