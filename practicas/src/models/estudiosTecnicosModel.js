...
class EstudiosTecnicosModel {
    ...
    update(id, newData) {
        ...
        if (match) {
            ...
            const type = newData.type || (/class="doc-type">([\s\S]*?)<\/span>/i.exec(oldHtml)?.[1] || 'ESTUDIO T&Eacute;CNICO');
            ...
            const href = newData.filename ? `../../data/menu header/la cas/talento humano/Estudios Tecnicos/${newData.filename}` : (/href="([^"]*)"/i.exec(oldHtml)?.[1] || '#');
