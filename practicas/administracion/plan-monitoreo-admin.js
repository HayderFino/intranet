...
const PlanMonitoreoAdmin = (() => {
    ...
    async function load() {
        ...
        elements.list.innerHTML = '<p style="padding:1rem;">Cargando archivos...</p>';
        try {
    ...
    function render() {
        ...
        if (items.length === 0) {
            elements.list.innerHTML = '<p style="padding:1rem;">No hay archivos registrados.</p>';
            return;
        }
    ...
    async function handleSubmit(e) {
        ...
        if (file) {
            ...
            if (FORBIDDEN_EXTS.includes(ext)) return notify('Tipo de archivo prohibido', 'error');
            if (file.size > MAX_SIZE_BYTES) return notify(`Supera el límite de ${MAX_SIZE_MB}MB`, 'error');
        }
    ...
    async function del(id) {
        if (id === 'sigep_default') return notify('El archivo principal no se puede eliminar por seguridad.', 'info');
