...
const ManualFuncionesAdmin = (() => {
    ...
    function init() {
        ...
        elements = {
            ...
            cancelContainer: document.getElementById('manualFuncionesCancelContainer')
        };

        if (!elements.form || !elements.list) {
            console.error('[ManualFuncionesAdmin] Elementos no encontrados en el DOM');
            return;
        }
    ...
    async function load() {
        ...
        elements.list.innerHTML = '<p style="padding:1rem;">Cargando manuales...</p>';
        try {
            ...
            console.error(e);
            elements.list.innerHTML = '<p style="padding:1rem; color:red;">Error al cargar el listado.</p>';
    ...
    function render() {
        ...
        if (items.length === 0) {
            elements.list.innerHTML = '<p style="padding:1rem;">No hay manuales registrados.</p>';
            return;
        }
    ...
    async function handleSubmit(e) {
        ...
        if (file) {
            ...
            if (FORBIDDEN_EXTS.includes(ext)) return showNotify('Tipo de archivo prohibido', 'error');
            if (file.size > MAX_SIZE_BYTES) return showNotify(`Supera el límite de ${MAX_SIZE_MB}MB`, 'error');
        }
        ...
        } catch (err) { showNotify('Error de red', 'error'); }
    }
    ...
    function showNotify(msg, type = 'success') {
        if (typeof showToast === 'function') {
            ...
            if (t) {
                t.innerText = msg;
                t.style.backgroundColor = type === 'success' ? '#10b981' : (type === 'error' ? '#ef4444' : '#3b82f6');
