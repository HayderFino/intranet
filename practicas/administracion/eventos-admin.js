const EventosAdmin = {
    ...
    load: async function () {
        ...
        try {
            ...
            if (items.length === 0) {
                list.innerHTML = '<p style="color:#64748b;padding:1rem;">No hay entradas registradas.</p>';
                return;
            }
            list.innerHTML = '';
            items.forEach(ev => {
    ...
    save: async function (e) {
        ...
        try {
            ...
            if (res.ok) { showToast(id ? 'Actualizado' : 'Guardado'); this.resetForm(); this.load(); }
    ...
    delete: async function (id) {
        ...
        if (res.ok) { showToast('Eliminado'); this.load(); }
