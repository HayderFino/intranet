...
const SirhAdmin = (() => {
    ...
    function render() {
        ...
        const filtered = filterVal === 'all' ? items : items.filter(i => i.category === filterVal);
        ...
        filtered.forEach(item => {
            ...
            // Icon based on type
            let icon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z"></path><polyline points="13 2 13 9 20 9"></polyline></svg>';
            const type = (item.type || 'PDF').toUpperCase();
            if (['JPG', 'PNG', 'SVG', 'JPEG'].includes(type)) icon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>';
            if (['XLS', 'XLSX'].includes(type)) icon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z"></path><path d="M8 13h8M8 17h8M8 9h2"></path></svg>';
    ...
    async function handleSubmit(e) {
        ...
        if (!id && !file) return showNotify('Selecciona un archivo para subir', 'error');
