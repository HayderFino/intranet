...
const regex = /<div [^>]*class="[^"]*news-item[^"]*"[^>]*>[\s\S]*?<img [^>]*src="([^"]*)"[^>]*>[\s\S]*?<h3[^>]*>([\s\S]*?)<\/h3>[\s\S]*?(?:<p[^>]*>([\s\S]*?)<\/p>)?[\s\S]*?<\/div>/g;
...
const filteredLegacy = newsItems.filter(ln => !existingNews.some(en => en.title === ln.title));
