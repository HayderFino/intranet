document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderResults(results, query) {
        ...
        resultsList.innerHTML = results.map((res, i) => {
            const isFile = res.href && (res.href.endsWith('.pdf') || res.href.endsWith('.docx') || res.href.endsWith('.pptx') || res.href.endsWith('.xlsx') || res.href.endsWith('.doc'));
            const target = isFile || res.href.startsWith('http') ? '_blank' : '_self';
    ...
    const ICONS = {
        noticia: '<svg style="width:12px;height:12px;" viewBox="0 0 24 24" fill="currentColor"><path d="M20 4H4a2 2 0 00-2 2v12a2 2 0 002 2h16a2 2 0 002-2V6a2 2 0 00-2-2zm-1 14H5V8h14v10z"/></svg>',
        ...
        calendar: '<svg style="width:12px;height:12px;" viewBox="0 0 24 24" fill="currentColor"><path d="M19 4h-1V2h-2v2H8V2H6v2H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V10h14v10zm0-12H5V6h14v2z"/></svg>',
        default: '<svg style="width:12px;height:12px;" viewBox="0 0 24 24" fill="currentColor"><path d="M10 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/></svg>',
    };
    ...
    function getTypeStyle(type) {
        ...
        for (const k of Object.keys(TYPE_STYLES)) {
            if (lt.includes(k)) { key = k; break; }
        }
        const style = TYPE_STYLES[key] || { color: '#64748b', bg: '#f1f5f9' };

        let icon = ICONS.default;
        if (lt.includes('noticia')) icon = ICONS.noticia;
        else if (lt.includes('agenda') || lt.includes('evento')) icon = ICONS.calendar;
        else if (lt.includes('herramienta')) icon = ICONS.tool;
        else if (lt.includes('manual') || lt.includes('informe') || lt.includes('sgi') ||
            lt.includes('documento') || lt.includes('git') || lt.includes('talento')) icon = ICONS.doc;
