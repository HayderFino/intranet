...
class InformeGestionModel {
    ...
    getAll() {
        ...
        const itemRegex = /<div class="card info-card"[^>]*?data-id="([^"]*)"[^>]*?>([\s\S]*?<\/div>[\s\S]*?<\/div>\s*<\/div>)/gi;
        ...
        while ((match = itemRegex.exec(content)) !== null) {
            ...
            const downloadMatch = /class="btn-pdf-download"[^>]*href="([^"]*)"/i.exec(inner);

            items.push({
                id,
                title: titleMatch ? this._decodeHTML(titleMatch[1].trim()) : 'Sin título',
                description: descMatch ? this._decodeHTML(descMatch[1].trim()) : '',
                pdfUrl: iframeMatch ? iframeMatch[1] : (downloadMatch ? downloadMatch[1] : '#')
            });
        }
    ...
    create(item) {
        ...
        if (index !== -1) {
            const insertIndex = content.indexOf('>', index + insertionPoint.length) + 1;
            // O mejor después de los breadcrumbs
            const breadcrumbEnd = ' </nav>';
            const bIndex = content.indexOf(breadcrumbEnd, insertIndex);
            const finalInsertIndex = bIndex !== -1 ? bIndex + breadcrumbEnd.length : insertIndex;

            content = content.substring(0, finalInsertIndex) + '\n' + itemHtml + content.substring(finalInsertIndex);
    ...
    update(id, newData) {
        ...
        const itemRegex = new RegExp(`<div class="card info-card"[^>]*?data-id="${escapedId}"[\\s\\S]*?<\\/div>[\\s\\S]*?<\\/div>\\s*<\\/div>`, 'i');
        ...
        if (match) {
            ...
            const pdfUrl = newData.filename ? `../../data/menu header/la cas/Informe de Gestión/${newData.filename}` : (/src="([^"]*?\.pdf)/i.exec(oldHtml)?.[1] || '#');
    ...
    delete(id) {
        ...
        if (match) {
            ...
            if (pdfMatch && pdfMatch[1] && pdfMatch[1] !== '#' && !pdfMatch[1].startsWith('http')) {
                const relativePath = decodeURIComponent(pdfMatch[1].split('?')[0].split('#')[0]).trim();
                const absolutePath = path.resolve(path.dirname(this.htmlPath), relativePath);
                try {
                    if (fs.existsSync(absolutePath)) fs.unlinkSync(absolutePath);
                } catch (e) { console.error('Error delete file:', e.message); }
            }
    ...
    _decodeHTML(html) {
        return html
            .replace(/&iacute;/g, 'í').replace(/&aacute;/g, 'á').replace(/&uacute;/g, 'ú').replace(/&oacute;/g, 'ó').replace(/&eacute;/g, 'é')
            .replace(/&Aacute;/g, 'Á').replace(/&Eacute;/g, 'É').replace(/&Iacute;/g, 'Í').replace(/&Oacute;/g, 'Ó').replace(/&Uacute;/g, 'Ú')
            .replace(/&ntilde;/g, 'ñ').replace(/&Ntilde;/g, 'Ñ').replace(/&iquest;/g, '¿').replace(/&ndash;/g, '-');
    }
}
