...
const HTML_PATH = path.join(__dirname, '../../header_menu/git/manuales_usuario/snif.html');

// Carpeta física donde se almacenan los archivos SNIF
const SNIF_DATA_DIR = path.join(__dirname, '../../data/menu header/git/manuales de usuario/Snif');
...
const SnifModel = {
    ...
    getAll: () => {
        ...
        if (startIndex !== -1) {
            ...
            if (nextSection === -1) nextSection = content.indexOf('</main>', startIndex);
    ...
    update: (id, data) => {
        ...
        const cardRegex = new RegExp(`<a [^>]*data-id="${escapedId}"[^>]*>[\\s\\S]*?<\\/a>`, 'g');
    ...
    delete: (id) => {
        ...
        if (match) {
            ...
            if (hrefMatch && hrefMatch[1] && hrefMatch[1] !== '#' && !hrefMatch[1].startsWith('http')) {
                ...
                const absolutePath = path.resolve(path.dirname(HTML_PATH), relativePath);
                try {
