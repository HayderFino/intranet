...
const DOC_EXTENSIONS = new Set(['.pdf', '.docx', '.doc', '.xlsx', '.xls', '.pptx', '.ppt', '.odt', '.ods', '.odp']);
...
function cleanHtml(str) {
    return (str || '')
        ...
        .replace(/&eacute;/gi, 'é').replace(/&iacute;/gi, 'í')
...
function normalizeUrl(href, htmlFilePath) {
    ...
    if (href.startsWith('../')) {
        ...
        const rel = path.relative(path.join(SERVER_ROOT), resolved).replace(/\\/g, '/');
        return '/' + rel;
    }
    // Relativa simple
    const htmlDir = path.dirname(htmlFilePath);
            ...
            if (entry.isDirectory()) {
                ...
                if (DOC_EXTENSIONS.has(ext)) {
                    ...
                    items.push({
                        ...
                        searchText: `${title} ${entry.name} ${parentDir} ${typeLabel} ${rel}`.toLowerCase()
                    });
                }
            }
        }
...
function crawlHtmlFiles() {
    ...
    function processHtml(htmlFilePath) {
        ...
        let content;
        try { content = fs.readFileSync(htmlFilePath, 'utf8'); }
        catch { return; }

        const htmlRelPath = path.relative(SERVER_ROOT, htmlFilePath).replace(/\\/g, '/');
        ...
        while ((fm = fileItemRegex.exec(content)) !== null) {
            ...
            const rawName = nameMatch ? cleanHtml(nameMatch[1]) : fileNameToTitle(path.basename(rawHref));
            ...
            items.push({
                ...
                searchText: `${rawName} ${cat} ${pageType} ${href} ${rawHref}`.toLowerCase()
            });
        }
    }
...
const UniversalCrawler = {
    ...
    getIndex() {
        ...
        _cacheTime = now;
        console.log(`[UniversalCrawler] Índice construido: ${_cache.length} documentos indexados.`);
        return _cache;
    },
