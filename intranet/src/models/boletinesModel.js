...
const BOLETINES_HTML_PATH = path.join(__dirname, '../../header_menu/git/boletines.html');
...
const BoletinesModel = {
    getAll: () => {
        ...
        const gridRegex = /<div[^>]*id="boletines-historico-grid"[^>]*>([\s\S]*?)<!-- END_BOLETINES_GRID -->/;
        ...
        while ((match = itemRegex.exec(gridContent)) !== null) {
            ...
            const subtitleMatch = /<span[^>]*font-size: 0.75rem[^>]*>([\s\S]*?)<\/span>/.exec(inner);

            items.push({
                id,
                title: titleMatch ? titleMatch[1].replace(/&iacute;/g, 'í').replace(/&oacute;/g, 'ó').trim() : '',
                subtitle: subtitleMatch ? subtitleMatch[1].replace(/&oacute;/g, 'ó').replace(/&oacute;/g, 'ó').trim() : '',
    ...
    create: (data) => {
        ...
        content = content.replace('<!-- END_BOLETINES_GRID -->', newItemHtml + '\n                <!-- END_BOLETINES_GRID -->');
    ...
    delete: (id) => {
        ...
        if (targetIndex !== -1) {
            ...
            if (hrefMatch && hrefMatch[1] && !hrefMatch[1].startsWith('http') && hrefMatch[1] !== '#') {
                ...
                const absolutePath = path.resolve(path.dirname(BOLETINES_HTML_PATH), relativePath);
