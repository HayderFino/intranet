...
class ConvocatoriasModel {
    constructor() {
        this.htmlPath = path.join(__dirname, '../../header_menu/cas/convocatorias.html');
        this.baseDataDir = path.join(__dirname, '../../data/menu header/la cas/talento humano/Convocatorias');
    ...
    getAll() {
        ...
        while ((match = itemRegex.exec(content)) !== null) {
            ...
            const descMatch = /class="doc-description">([\s\S]*?)<\/div>/i.exec(inner);
    ...
    create(item) {
        ...
        if (index !== -1) {
            ...
            content = content.substring(0, insertIndex) + '\n' + itemHtml + content.substring(insertIndex);
    ...
    update(id, newData) {
        ...
        const itemRegex = new RegExp(`<div class="doc-item"[^>]*?data-id="${escapedId}"[\\s\\S]*?<\\/a>\\s*<\\/div>`, 'i');
        const match = itemRegex.exec(content);

        if (match) {
            const oldHtml = match[0];
            const type = newData.type || (/class="doc-type">([\s\S]*?)<\/span>/i.exec(oldHtml)?.[1] || 'DOCUMENTO');
            const date = newData.date || (/class="doc-date">([\s\S]*?)<\/span>/i.exec(oldHtml)?.[1] || '');
            const title = newData.title || (/class="doc-title">([\s\S]*?)<\/div>/i.exec(oldHtml)?.[1] || '');
            const description = newData.description || (/class="doc-description">([\s\S]*?)<\/div>/i.exec(oldHtml)?.[1] || '');
            const href = newData.filename ? `../../data/menu header/la cas/talento humano/Convocatorias/${newData.filename}` : (/href="([^"]*)"/i.exec(oldHtml)?.[1] || '#');
