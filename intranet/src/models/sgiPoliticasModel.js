...
const UPLOAD_DIR = path.join(process.cwd(), 'data/menu header/sgi/Políticas Institucionales');
...
const SgiPoliticasModel = {
    ...
    getAll() {
        ...
        const articleRx = /<article[^>]*class="policy-card[^"]*"[^>]*data-id="([^"]*)"[^>]*>([\s\S]*?)<\/article>/g;
        let artMatch;
        while ((artMatch = articleRx.exec(content)) !== null) {
            const id = artMatch[1];
            const chunk = artMatch[2];
            const titleM = /<h2[^>]*class="policy-title"[^>]*>([\s\S]*?)<\/h2>/.exec(chunk);
            const hrefM = /<a [^>]*href="([^"#][^"]*)"[^>]*class="btn-download"/.exec(chunk)
        ...
        const gridM = /(<!-- BEGIN_POLITICAS_DOCS -->)([\s\S]*?)(<!-- END_POLITICAS_DOCS -->)/.exec(content);
        if (gridM) {
            ...
            for (let i = 1; i < parts.length; i++) {
                ...
                const hrefM = /href="([^"]*)"[^>]*class="btn-download"/.exec(chunk)
    ...
    create(data) {
        ...
        content = content.replace('<!-- END_POLITICAS_DOCS -->', card + '\n                <!-- END_POLITICAS_DOCS -->');
    ...
    delete(id) {
        ...
        if (id.startsWith('static-')) {
            const articleRegex = new RegExp(`<article[^>]*data-id="${id}"[\\s\\S]*?<\\/article>`, 'i');
            ...
            const newParts = parts.filter((p, i) => {
                ...
                if (p.includes(`data-id="${id}"`)) {
                    const hrefM = /href="([^"]*)"[^>]*class="btn-download"/.exec(p)
                        || /class="btn-download"[^>]*href="([^"]*)"/.exec(p);
                    if (hrefM && hrefM[1] !== '#' && hrefM[1].includes('data/')) {
                        try {
                            const abs = path.join(process.cwd(), hrefM[1].replace('../../', ''));
            ...
            if (found) {
                const newGrid = newParts.map((p, i) => i === 0 ? p : `<article${p}`).join('');
    ...
    update(id, data) {
        ...
        if (id.startsWith('static-')) {
            ...
            const articleRegex = new RegExp(`(<article[^>]*data-id="${id}"[\\s\\S]*?<h2[^>]*class="policy-title"[^>]*>)([\\s\\S]*?)(<\\/h2>[\\s\\S]*?<div[^>]*class="policy-content"[^>]*>\\s*<p>)([\\s\\S]*?)(<\\/p>[\\s\\S]*?class="btn-download"[^>]*href=")([^"]*)`, 'i');

            if (articleRegex.test(content)) {
                content = content.replace(articleRegex, `$1${data.title}$3${data.code || ''}$5${data.fileUrl || '#'}`);
                fs.writeFileSync(HTML_PATH, content, 'utf8');
                return id;
            } else {
                // Fallback: al menos actualizar el href si la estructura es distinta
                const hrefOnlyRegex = new RegExp(`(<article[^>]*data-id="${id}"[\\s\\S]*?class="btn-download"[^>]*href=")([^"]*)`, 'i');
