...
const SIRH_HTML_PATH = path.join(__dirname, '../../header_menu/git/manuales_usuario/sirh.html');
    ...
    delete: (id) => {
        ...
        if (match) {
            ...
            if (hrefMatch && hrefMatch[1] && hrefMatch[1] !== '#' && !hrefMatch[1].startsWith('http')) {
                ...
                const absolutePath = path.resolve(path.dirname(SIRH_HTML_PATH), relativePath);
                try {
