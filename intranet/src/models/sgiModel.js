...
const CONFIG = {
    ...
    'gestion-documental': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/gestion-documental.html')
    },
    'gestion-financiera': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/gestion-financiera.html')
    },
    'gestion-tecnologias': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/gestion-tecnologias.html')
    },
    'juridico': {
    ...
    'gestion-integral': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/gestion-integral.html')
    },
    'mejora-continua': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/mejora-continua.html')
    },
    'procesos-estrategicos': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/procesos-estrategicos.html')
    },
    'procesos-misionales': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/procesos-misionales.html')
    },
    'procesos-apoyo': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/procesos-apoyo.html')
    },
    'control-disciplinario': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/control-disciplinario.html')
    },
    'cobro-coactivo': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/cobro-coactivo.html')
    },
    'bienes-servicios': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/bienes-servicios.html')
    },
    'objetivos-calidad': {
        htmlPath: path.join(__dirname, '../../header_menu/sgi/objetivos-calidad.html')
    },
    'politicas': {
...
const SgiModel = {
    ...
    getAll: (section) => {
        ...
        try {
            ...
            console.error(`SgiModel.getAll: cannot read ${config.htmlPath}`, e.message);
            return [];
        }
        ...
        while ((catMatch = categoryRegex.exec(content)) !== null) {
            ...
            while ((itemMatch = itemRegex.exec(gridContent)) !== null) {
                ...
                const nameMatch = /<div class="file-name">([\s\S]*?)<\/div>/.exec(innerHtml);
                ...
                } else if (fileUrl !== '#' && !fileUrl.startsWith('/') && !fileUrl.startsWith('http')) {
    ...
    getAllSections: () => {
        ...
        for (const section of Object.keys(CONFIG)) {
            try {
                ...
                items.forEach(item => {
                    allItems.push({ ...item, section, sectionLabel: SECTION_LABELS[section] || 'SGI' });
                });
            } catch (e) {
                console.error(`SgiModel.getAllSections error in ${section}:`, e.message);
            }
        }
        return allItems;
    },
    ...
    create: (section, name, category, fileUrl, existingId = null) => {
        ...
        else if (lc.includes('riesgo') || lc.includes('matriz')) meta = 'PDF - Matriz de Riesgos';
        else if (lc.includes('anexo') || lc.includes('guía')) meta = 'PDF - Anexo';
        else if (lc.includes('membrete') || lc.includes('papelería')) meta = 'Word - Institucional';
        ...
        const fixForHtml = (str) => str
            .replace(/ó/g, '&oacute;').replace(/á/g, '&aacute;').replace(/é/g, '&eacute;')
            .replace(/í/g, '&iacute;').replace(/ú/g, '&uacute;').replace(/ñ/g, '&ntilde;');

        const searchCategory = fixForHtml(category);
        const escapedCategory = searchCategory.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const catRegex = new RegExp(`(<h3[^>]*>${escapedCategory}<\\/h3>[\\s\\S]*?<div [^>]*class="file-list-grid"[^>]*>)`, 'i');
        ...
        if (catRegex.test(content)) {
            ...
            const rawCatRegex = new RegExp(`(<h3[^>]*>${rawEscaped}<\\/h3>[\\s\\S]*?<div [^>]*class="file-list-grid"[^>]*>)`, 'i');
