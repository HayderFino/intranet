...
const STATIC_PAGES = [
    // ── La CAS ──
    {
        ...
        type: 'La CAS',
        snippet: 'Misión y visión institucional de la Corporación Autónoma Regional de Santander.'
    },
    {
        ...
        type: 'La CAS',
        snippet: 'Objetivos institucionales de la Corporación Autónoma Regional de Santander.'
    },
    {
    ...
    {
        keywords: ['procesos estratégicos', 'procesos estrategicos', 'estratégicos'],
    ...
    {
        keywords: ['documentos institucionales', 'membrete', 'papelería', 'membrete carta', 'membrete oficio', 'identidad corporativa', 'logo'],
        title: 'Documentos Institucionales',
        href: '/header_menu/sgi/documentos.html',
        type: 'SGI',
        snippet: 'Plantillas, membretes y documentos de identidad corporativa de la CAS.'
    },
    {
    ...
    {
        keywords: ['bienes', 'servicios', 'bienes y servicios', 'contratación bienes'],
    ...
    {
        keywords: ['gestión documental', 'gestion documental', 'archivo', 'documentación'],
    ...
    {
        keywords: ['gestión financiera', 'gestion financiera', 'finanzas', 'presupuesto'],
    ...
    {
        keywords: ['gestión de tecnologías', 'gestion de tecnologias', 'tecnologías', 'tic', 'sistemas'],
    ...
    {
        keywords: ['planeación estratégica', 'planeacion estrategica', 'plan estratégico'],
    ...
    {
        keywords: ['planeación ambiental', 'planeacion ambiental', 'ordenamiento ambiental'],
    ...
    {
        keywords: ['vigilancia', 'control ambiental', 'seguimiento ambiental', 'vigilancia y control'],
    ...
    {
        keywords: ['control interno', 'auditoria', 'auditoría', 'control interno pci'],
    ...
    {
        keywords: ['talento humano sgi', 'gestión del talento', 'gestion del talento'],
    ...
    // ── GIT ──
    {
        keywords: ['normatividad git', 'normatividad tecnologías', 'normatividad tic'],
    ...
    {
        keywords: ['responsables de la información', 'responsables pagina web', 'responsables web'],
        title: 'Responsables de la Información en Página WEB',
        href: '/header_menu/git/responsables.html',
        type: 'GIT',
        snippet: 'Responsables de la actualización de información en la página web.'
    },
    {
        keywords: ['protección de datos', 'proteccion de datos', 'datos personales', 'habeas data'],
    ...
    },
    {
        ...
        type: 'Manual Usuario',
        snippet: 'Manual de usuario del Sistema de Información de Recursos Humanos.'
    },
    {
    ...
    {
        keywords: ['revisión de red', 'revision de red', 'red primer nivel', 'redes sedes'],
    ...
    {
        keywords: ['cartografía', 'cartografia', 'mapa', 'sig', 'gis', 'visor geográfico'],
    ...
    // ── Talento Humano CAS ──
    {
        keywords: ['manual de funciones', 'manual funciones', 'funciones y competencias', 'cargos'],
    ...
    {
        keywords: ['plan de monitoreo sigep', 'plan monitoreo', 'sigep', 'monitoreo sigep'],
    ...
    {
        keywords: ['provisión de empleos', 'provision de empleos', 'provisión empleo', 'provisión'],
    ...
    {
        keywords: ['desprendibles de nómina', 'desprendibles nomina', 'nómina', 'nomina'],
    ...
    // ── MECI ──
    {
        keywords: ['meci', 'modelo estándar de control', 'modelo de control interno'],
...
function matchesStatic(query, pageKeywords) {
    ...
    return pageKeywords.some(kw => lq.includes(kw.toLowerCase()) || kw.toLowerCase().includes(lq));
}
    ...
    search: async (req, res) => {
        try {
            ...
            if (!category || category === 'all' || category === 'noticias') {
                ...
                } catch (e) { console.error('Search error (News):', e.message); }
            }
            ...
            if (!category || category === 'all' || category === 'informes') {
                ...
                } catch (e) { console.error('Search error (Informes):', e.message); }
            }

            // ── 3. MODELOS ESTRUCTURADOS CON ARCHIVOS ────────────────────
            if (!category || category === 'all' || category === 'manuales') {
                const manualModels = [
                    { model: CitaModel, type: 'Manual CITA', href: '/header_menu/git/manuales_usuario/cita.html' },
                    { model: SirhModel, type: 'Manual SIRH', href: '/header_menu/git/manuales_usuario/sirh.html' },
                    { model: SnifModel, type: 'Manual SNIF', href: '/header_menu/git/manuales_usuario/snif.html' },
                    { model: RevisionRedModel, type: 'Revisión de Red', href: '/header_menu/git/manuales_usuario/revision-red.html' },
                    { model: ManualFuncionesModel, type: 'Manual de Funciones', href: '/header_menu/cas/manual-funciones.html' },
                    { model: PlanMonitoreoModel, type: 'Plan Monitoreo SIGEP', href: '/header_menu/cas/plan-monitoreo-sigep.html' },
                    { model: PlanesTalentoModel, type: 'Planes Talento Humano', href: '/header_menu/cas/planes.html' },
                    { model: ConvocatoriasModel, type: 'Convocatoria', href: '/header_menu/cas/convocatorias.html' },
                    { model: EstudiosTecnicosModel, type: 'Estudios Técnicos', href: '/header_menu/cas/estudios-tecnicos.html' },
                    { model: ProvisionEmpleosModel, type: 'Provisión de Empleos', href: '/header_menu/cas/provision-empleos.html' },
                    { model: BoletinesModel, type: 'Boletín de Seguridad', href: '/header_menu/git/boletines.html' },
                    { model: ManualesModel, type: 'Manual SGI', href: '/header_menu/sgi/manuales.html' },
                    { model: PcbModel, type: 'PCB', href: '/herramientas/pcb.html' },
                    { model: RespelModel, type: 'RESPEL', href: '/herramientas/respel.html' },
                ...
                for (const mObj of manualModels) {
                    ...
                    } catch (e) { console.error(`Search error (${mObj.type}):`, e.message); }
                }
                ...
                } catch (e) { console.error('Search error (SGI All):', e.message); }
                ...
                } catch (e) { console.error('Search error (Agenda):', e.message); }
                ...
                } catch (e) { console.error('Search error (UniversalCrawler):', e.message); }
            }
            ...
            if (keywords.length > 0) {
                STATIC_PAGES.forEach(page => {
                    if (matchesStatic(query, page.keywords)) {
                        const alreadyIn = results.some(r => r.href === page.href && r.title === page.title);
                        if (!alreadyIn) {
                            results.push({
                                title: page.title,
                                type: page.type,
