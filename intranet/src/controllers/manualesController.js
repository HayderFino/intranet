...
const ManualesController = {
    getItems: (req, res) => {
        try { res.json(ManualesModel.getAll()); }
        catch (e) { res.status(500).json({ message: 'Error al obtener manuales.' }); }
    },
    createItem: (req, res) => {
        try {
    ...
    updateItem: (req, res) => {
        try {
            ...
            if (newId) res.status(200).json({ message: 'Manual actualizado.', id: newId });
