...
const PcbController = {
    ...
    // --- Tabla de Plazos ---
    getAllRows: (req, res) => {
        try { res.json(PcbModel.getAllRows()); }
        catch (e) { res.status(500).json({ message: 'Error al obtener filas.' }); }
    },
    createRow: (req, res) => {
        try {
    ...
    updateRow: (req, res) => {
        try {
            ...
            if (newId) res.status(200).json({ message: 'Fila actualizada.', id: newId });
