...
const RevisionRedAdmin = (() => {
    ...
    // --- Core Actions ---
    async function load() {
        elements.list.innerHTML = '<p style="padding:1rem; color:#64748b;">Cargando archivos...</p>';
        try {
            ...
            elements.list.innerHTML = '<p style="padding:1rem; color:#ef4444;">Error de conexión con el servidor.</p>';
        }
    }
    ...
    async function handleSubmit(e) {
        ...
        if (!id && !file) return showNotify('Debes seleccionar un archivo para subir', 'error');
        if (!elements.name.value.trim()) return showNotify('El nombre del archivo es obligatorio', 'error');
        ...
        try {
            ...
            if (res.ok) {
                showNotify(id ? 'Archivo actualizado correctamente' : 'Archivo subido con éxito');
                resetForm();
                load();
            } else {
    ...
    async function del(id) {
        if (!confirm('¿Seguro que deseas eliminar este archivo? Esta acción no se puede deshacer.')) return;
        try {
