const DirectorioAdmin = {
    ...
    load: async function () {
        ...
        try {
            ...
            if (items.length === 0) { list.innerHTML = '<p style="color:#64748b;padding:1rem;">No hay contactos registrados.</p>'; return; }
            list.innerHTML = '';
            items.forEach(p => {
        ...
        } catch (e) { showToast('Error al cargar directorio', 'error'); }
    },
    ...
    save: async function (e) {
        ...
        try {
            ...
            if (res.ok) { showToast(id ? 'Contacto actualizado' : 'Contacto creado'); this.resetForm(); this.load(); }
    ...
    edit: async function (id) {
        ...
        document.getElementById('directorioForm').scrollIntoView({ behavior: 'smooth' });
    },

    delete: async function (id) {
        if (!confirm('¿Eliminar este contacto?')) return;
        const res = await fetch(`${this.API}/${id}`, { method: 'DELETE' });
        if (res.ok) { showToast('Contacto eliminado'); this.load(); }
