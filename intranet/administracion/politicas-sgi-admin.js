...
const PoliticasSgiAdmin = (() => {
    ...
    let form, editId, titleInput, codeInput, fileInput,
        listEl, saveBtn, cancelBtn;
    ...
    async function load() {
        ...
        try {
            ...
            listEl.innerHTML = '<p style="color:#ef4444;padding:1rem;">Error al cargar: ' + e.message + '</p>';
        }
    }
    ...
    function renderList() {
        ...
        if (items.length === 0) {
            ...
            empty.style.cssText = 'color:#94a3b8;font-size:0.85rem;padding:2rem;text-align:center;background:#f8fafc;border-radius:12px;border:1px dashed #e2e8f0;';
        ...
        listEl.querySelectorAll('.btn-delete').forEach(btn => btn.onclick = () => deleteItem(btn.dataset.id));
        listEl.querySelectorAll('.btn-edit').forEach(btn => btn.onclick = () => startEdit(items.find(i => i.id === btn.dataset.id)));
    }
    ...
    async function onSubmit(e) {
        ...
        const payload = { title: titleInput.value.trim(), code: codeInput.value.trim(), fileUrl };
    ...
    async function deleteItem(id) {
        ...
        try {
            ...
            if (res.ok) { toast('Documento eliminado'); load(); }
